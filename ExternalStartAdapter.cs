    ...
    public static class ExternalStartAdapter
    {
        ...
        private static void TryLoadByPath(ISldWorks swApp)
        {
            try
            {
                ...
                // Optional explicit path (none configured by default). Set NM_ExtractAddInPath env var if needed.
                var path = System.Environment.GetEnvironmentVariable("NM_ExtractAddInPath");
                if (string.IsNullOrWhiteSpace(path)) { ErrorHandler.DebugLog("[ExternalStart] Add-in path env var not set."); return; }
