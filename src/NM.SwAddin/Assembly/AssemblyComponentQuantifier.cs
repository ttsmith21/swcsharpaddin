    ...
    public sealed class AssemblyComponentQuantifier
    {
        ...
        public Dictionary<string, ComponentQuantity> TryCollectViaBom(IAssemblyDoc asm, string bomTemplatePath = "")
        {
            ...
            try
            {
                ...
                bom = ext.InsertBomTable3(bomTemplatePath ?? string.Empty, 0.0, 0.0,
                    (int)swBomType_e.swBomType_PartsOnly, cfgname: Safe(() => model.ConfigurationManager?.ActiveConfiguration?.Name) ?? "",
                    displayQuantity: false, (int)swNumberingType_e.swIndentedBOMNotSet, showDetailedCutList: false) as IBomTableAnnotation;
                ...
                int rows = Safe(() => table.RowCount) ?? 0;
                ...
                for (int r = 1; r < rows; r++)
                {
                    ...
                    try
                    {
                        // Prefer GetComponentsInRow2; fall back to GetComponents2
                        try { compsObj = bom.GetComponentsInRow2(r); }
                        catch { compsObj = null; }
                        if (compsObj == null)
                        {
                            try { compsObj = bom.GetComponents2(r); } catch { compsObj = null; }
                        }
            ...
            finally
            {
                ...
                try
                {
                    if (bomFeature != null)
                    {
                        ...
                        model.Extension.DeleteSelection2((int)swDeleteSelectionOptions_e.swDelete_Silent);
                        try { model.ForceRebuild3(true); } catch { }
                    }
                }
                catch { }
